...
const USEFUL_LINKS = [
  { label: 'Coupa Invoices', url: 'https://disney.coupahost.com/invoices', icon: 'ti-file-invoice', color: 'var(--brand)', bg: 'var(--brand-tint)' },
...
const HELP_ITEMS: HelpItem[] = [
  { title: '¿Cómo registrar una tarea?', icon: 'ti-clipboard-plus', content: 'En la pestaña "Tareas", hacé clic en "Registrar tarea". Buscá y seleccioná la tarea, completá el horario de inicio y fin, el sistema y la etiqueta. La duración se calcula automáticamente.' },
...
export default function HelpSection() {
  ...
  return (
    <div style={font}>
      ...
      <div style={{ background: 'linear-gradient(135deg, var(--brand) 0%, var(--brand-dark) 100%)', borderRadius: 14, padding: '16px', marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {USEFUL_LINKS.map(l => (
            <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer"
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', flex: 1, lineHeight: 1.2 }}>{l.label}</span>
              <i className="ti ti-external-link" style={{ fontSize: 13, color: 'var(--text3)' }} />
            </a>
          ))}
        </div>
      </div>
      ...
      {HELP_ITEMS.map((item, i) => (
        <div key={i} style={{ marginBottom: 8 }}>
          <button onClick={() => setOpen(open === i ? null : i)}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: open === i ? '10px 10px 0 0' : 10, border: `0.5px solid ${open === i ? 'var(--brand)' : 'var(--border)'}`, background: open === i ? 'var(--brand-tint)33' : 'var(--card)', cursor: 'pointer', ...font }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: 'var(--brand-tint)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className={`ti ${item.icon}`} style={{ fontSize: 14, color: 'var(--brand-dark)' }} />
              </div>
              <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', textAlign: 'left' }}>{item.title}</span>
            </div>
            <i className={`ti ${open === i ? 'ti-chevron-up' : 'ti-chevron-down'}`} style={{ fontSize: 15, color: open === i ? 'var(--brand)' : 'var(--text3)' }} />
          </button>
          {open === i && (
            <div style={{ border: '0.5px solid var(--brand)', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: '12px 16px', background: 'var(--card)' }}>
