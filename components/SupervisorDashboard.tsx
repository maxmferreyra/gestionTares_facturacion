...
export default function SupervisorDashboard() {
  ...
  const allUserIds = new Set([...Object.keys(actionsByCollab), ...Object.keys(tasksByCollab)])
    ...
    <div style={font}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
        {[
          ...
          <div key={s.label} style={{ background: 'var(--card)', borderRadius: 10, padding: '12px', border: '0.5px solid var(--border)', textAlign: 'center' }}>
          ...
          return (
            <div key={sys.key} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, height: 10, background: 'var(--bg)', borderRadius: 99, overflow: 'hidden' }}>
                <div style={{ width: `${(count / maxSys) * 100}%`, height: '100%', background: sys.color, borderRadius: 99, transition: 'width .4s' }} />
              </div>
              <span style={{ fontSize: 13, fontWeight: 600, color: sys.color, minWidth: 28, textAlign: 'right' }}>{count}</span>
            </div>
          )
        })}
        {totalActions === 0 && <div style={{ fontSize: 13, color: 'var(--text4)', fontWeight: 300, textAlign: 'center', padding: '1rem 0' }}>Sin acciones en este período</div>}
      </div>
      ...
      <div style={{ background: 'var(--card)', borderRadius: 12, border: '0.5px solid var(--border)', padding: '14px', marginBottom: 12 }}>
            ...
            <div key={id} style={{ marginBottom: 14, paddingBottom: 14, borderBottom: '0.5px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                {avatar
                  ...
                  : <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#CECBF6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: '#3C3489', flexShrink: 0 }}>{getInitials(id)}</div>}
                <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', flex: 1 }}>{getName(id)}</span>
                <span style={{ fontSize: 11, color: 'var(--text3)' }}>{tsks} tareas</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: 1, height: 8, background: 'var(--bg)', borderRadius: 99, overflow: 'hidden' }}>
                  <div style={{ width: `${(acts / maxCollab) * 100}%`, height: '100%', background: '#534AB7', borderRadius: 99, transition: 'width .4s' }} />
                </div>
        ...
        {allUserIds.size === 0 && <div style={{ fontSize: 13, color: 'var(--text4)', fontWeight: 300, textAlign: 'center', padding: '1rem 0' }}>Sin actividad en este período</div>}
      </div>
